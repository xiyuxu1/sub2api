<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select v-if="showOwnerFilter" :model-value="filters.owner" class="w-52" :options="ownerOpts" @update:model-value="updateOwner" @change="$emit('change')" />
  </div>
</template>
...
const props = defineProps<{ searchQuery: string; filters: Record<string, any>; groups?: AdminGroup[]; users?: AdminUser[]; showOwnerFilter?: boolean }>()
...
const updateOwner = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, owner: value }) }
...
const ownerOptionLabel = (user: AdminUser) => {
  const name = user.username && user.username !== user.email
    ? `${user.username} · ${user.email}`
    : user.email
  return user.deleted_at ? t('admin.accounts.owner.deleted', { name }) : name
}
const ownerOpts = computed(() => [
  { value: '', label: t('admin.accounts.allOwners') },
  { value: 'unassigned', label: t('admin.accounts.systemOwnedAccounts') },

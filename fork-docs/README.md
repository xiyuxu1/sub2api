...
>
> **当前一句话状态（2026-07-19）**：普通用户「我的账号」已复用完整账号管理页，具备搜索、手动/自动刷新、实时用量/次数、编辑、测试及共享代理/分组选择；按用户明确选择的 B 信任模型放开共享绑定，同时保留 owner 收口、脱敏 DTO 与 admin-only 操作隔离。代码已完成并通过全量前端测试、后端相关包回归及独立安全审查，待发布 `0.1.161-xdl4`。
...
> ✅ **已完成（2026-07-19，待部署 `0.1.161-xdl4`）**：用户明确选择 **B（信任模型放宽）**。`/my-accounts` 直接复用 `AccountsView.vue` 的 self-service 模式，保留搜索、筛选、手动/自动刷新、今日次数、实时用量、编辑、测试、重新授权、刷新 Token、公开/私有、代理和分组选择；隐藏批量操作、调度开关、CRS、计费探测、配额重置等 admin-only 功能。后端新增脱敏的共享代理/分组选项接口，开放必要 OAuth/统计/配额路由并逐账号执行 owner 校验；混合渠道预检也先校验 owner 并使用真实平台，批量今日统计限制 200 个 ID。普通用户新建账号仍 `schedulable=false`，等待 admin 审核。验证：前端 175 文件/1220 测试、`vue-tsc`、生产构建、后端相关四包全量测试及 `go vet` 均通过；独立审查反馈已清零。

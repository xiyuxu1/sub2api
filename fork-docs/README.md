...
>
> **当前一句话状态（2026-07-19）**：普通用户「我的账号」已复用完整账号管理页，具备搜索、手动/自动刷新、实时用量/次数、编辑、测试及共享代理/分组选择；OpenAI/Codex、Grok 创建流程与临时账号 JSON 导入也已开放，并保留 owner 收口、代理基础设施隔离和 admin-only 操作隔离。国内生产已部署 `0.1.161-xdl5`，健康检查和两个公网入口均正常。

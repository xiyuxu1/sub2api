...
>
> **当前一句话状态（2026-07-19）**：普通用户「我的账号」完整自助、OpenAI/Codex/Grok 创建流程及临时账号 JSON 导入已随国内生产 `0.1.161-xdl5` 上线；管理员账号列表的“归属人”展示与筛选已在本地完成、待提交部署。线上容器健康，两个公网入口均正常。
...
- **管理员识别账号归属人已在本地完成，待提交/部署**（2026-07-19）：账号 service/DTO 已透传 `owner_user_id`；管理员账号表新增“归属人”列，普通用户账号显示用户名与邮箱，已删除用户有明确标记，历史无 owner 的账号显示“系统/管理员”；self-service 页面不展示该列，也不加载用户目录。管理员筛选栏新增归属人下拉，选项顺序固定为“全部归属人”“系统/管理员账号”（即 `owner_user_id IS NULL`）和全部普通用户（含已删除用户，用户名 + 邮箱）。Spark 影子账号继承母账号的 owner 与公开属性。`owner` 条件在 repository 中执行，已同步覆盖分页总数、自动刷新 ETag 请求、调度分候选池、按筛选批量编辑和按筛选导出，避免列表范围与写操作/导出范围不一致。验证：handler/DTO/service 测试、带 `unit` 标签的批量筛选测试、repository owner 映射测试、前端归属筛选与管理员表格测试、`vue-tsc` 均通过；repository testcontainers 集成用例已补，但本机下载依赖时网络超时，尚未实际启动。

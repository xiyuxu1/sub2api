...
func (h *AccountHandler) listAccountsFiltered(ctx context.Context, platform, accountType, status, search string, groupID, ownerUserID int64, privacyMode, sortBy, sortOrder string) ([]service.Account, error) {
	page := 1
	pageSize := dataPageCap
	var out []service.Account
	for {
		items, total, err := h.adminService.ListAccounts(ctx, page, pageSize, platform, accountType, status, search, groupID, ownerUserID, privacyMode, sortBy, sortOrder)
		if err != nil {
			return nil, err
		}
		out = append(out, items...)
...
func (h *AccountHandler) resolveExportAccounts(ctx context.Context, ids []int64, c *gin.Context) ([]service.Account, error) {
	...
	if err != nil {
		return nil, infraerrors.BadRequest("INVALID_OWNER_FILTER", "invalid owner filter")
	}

	return h.listAccountsFiltered(ctx, platform, accountType, status, search, groupID, ownerUserID, privacyMode, sortBy, sortOrder)
}

...
type AdminService interface {
	...
	// Account management
	ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID, ownerUserID int64, privacyMode string, sortBy, sortOrder string) ([]Account, int64, error)
	// ListPublicAccounts 返回其他人公开（is_public）的账号（fork），供非 admin 只读浏览。
	ListPublicAccounts(ctx context.Context, page, pageSize int, platform, search string) ([]Account, int64, error)
	// ListAccountsForSchedulerScoreFilter 返回符合过滤条件的全部账号（不分页），
	// 作为账号列表页计算 OpenAI 调度分数的过滤范围池。
	ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID, ownerUserID int64, privacyMode string) ([]Account, error)
...
type UpdateAccountInput struct {
	...
	ExpiresAt             *int64
	AutoPauseOnExpired    *bool
	IsPublic              *bool // fork: 管理可见性开关；nil = 不改。见 fork-docs/README.md §8.2。
